...
mod tests {
    ...
    fn test_xor_encryption_roundtrip() {
        ...
        assert_eq!(original_text.as_slice(), decrypted_data.as_slice());
    }
}
use aes_gcm::{
...
mod tests {
    ...
    fn test_tampered_data_fails() {
        ...
        let mut encrypted = encrypt_file_data(test_data).unwrap();
        encrypted.ciphertext[5] ^= 0xFF;
        
        let key = Aes256Gcm::generate_key(&mut OsRng);
        let result = decrypt_file_data(&encrypted, &key);
...
mod tests {
    ...
    fn test_xor_encryption_decryption() {
        let original_text = b"Hello, Rust! This is a test file.";
