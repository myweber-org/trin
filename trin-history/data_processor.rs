...
mod tests {
    ...
    fn test_data_processor() {
        ...
        assert!(avg.is_some());
        assert!((avg.unwrap() - 200.5).abs() < 0.001);
        
        let type_a_records = processor.filter_by_category("TypeA");
        assert_eq!(type_a_records.len(), 2);
        
        let stats = processor.get_statistics();
        assert!((stats.0 - 100.5).abs() < 0.001);
        assert!((stats.1 - 300.25).abs() < 0.001);
        assert!((stats.2 - 200.5).abs() < 0.001);
    }
    ...
    fn test_invalid_data_skipping() {
        ...
        let mut temp_file = NamedTempFile::new().unwrap();
        writeln!(temp_file, "id,value,category").unwrap();
        writeln!(temp_file, "1,100.5,Valid").unwrap();
        writeln!(temp_file, "0,200.75,InvalidID").unwrap();
        writeln!(temp_file, "2,-50.0,NegativeValue").unwrap();
        writeln!(temp_file, "3,15000.0,TooLarge").unwrap();
        writeln!(temp_file, "4,250.5,").unwrap();
...
impl DataProcessor {
    ...
    pub fn get_average_value(&self) -> f64 {
        ...
        if valid_count > 0 {
            self.total_value / valid_count as f64
        } else {
            0.0
        }
    }
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        assert!((average.unwrap() - 75.25).abs() < 0.001);
