...
impl DataProcessor {
    ...
    pub fn add_record(&mut self, record: DataRecord) -> Result<(), DataError> {
        ...
        if !self.processed_ids.insert(record.id) {
...
impl DataProcessor {
    ...
    pub fn calculate_average(&self, category: &str) -> Option<f64> {
        let filtered_records: Vec<&DataRecord> = self.records
            .values()
        ...
        Some(sum / filtered_records.len() as f64)
    }
    ...
    pub fn extract_column(&self, records: &[Vec<String>], column_index: usize) -> Result<Vec<String>, String> {
        ...
        for record in records {
            if column_index >= record.len() {
                return Err(format!("Column index {} out of bounds for record with {} columns", 
                    column_index, record.len()));
            }
...
mod tests {
    ...
    fn test_extract_column() {
        ...
        assert_eq!(column, vec!["apple", "banana"]);
    }
}