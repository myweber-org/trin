...
impl fmt::Display for ValidationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ValidationError::InvalidLength(min, max) => write!(f, "Length must be between {} and {} characters", min, max),
            ValidationError::InvalidFormat(expected) => write!(f, "Invalid format, expected: {}", expected),
            ValidationError::OutOfRange(value, min, max) => write!(f, "Value {} is out of range [{}, {}]", value, min, max),
        }
    }
}
...
mod tests {
    ...
    fn test_validate_string() {
        ...
        assert_eq!(DataValidator::validate_string("", 1, 10), Err(ValidationError::EmptyValue));
        assert_eq!(DataValidator::validate_string("a", 2, 10), Err(ValidationError::InvalidLength(2, 10)));
    }
    
    #[test]
    fn test_validate_email() {
        assert!(DataValidator::validate_email("user@example.com").is_ok());
        assert_eq!(DataValidator::validate_email("invalid"), Err(ValidationError::InvalidFormat("user@domain.com".to_string())));
        assert_eq!(DataValidator::validate_email("@domain.com"), Err(ValidationError::InvalidFormat("user@domain.com".to_string())));
    }
    
    #[test]
    fn test_validate_numeric_range() {
        assert!(DataValidator::validate_numeric_range(5.0, 0.0, 10.0).is_ok());
        assert_eq!(DataValidator::validate_numeric_range(15.0, 0.0, 10.0), Err(ValidationError::OutOfRange(15.0, 0.0, 10.0)));
    }
}