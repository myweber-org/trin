...
impl FileEncryptor {
    ...
    pub fn encrypt_file(&self, input_path: &Path, output_path: &Path) -> Result<(), EncryptionError> {
        if !input_path.is_file() {
            return Err(EncryptionError::InvalidFileType);
        }
    ...
    pub fn decrypt_file(&self, input_path: &Path, output_path: &Path) -> Result<(), EncryptionError> {
        ...
        let plaintext = match self.algorithm {
            EncryptionAlgorithm::Aes256Gcm => self.decrypt_aes(ciphertext, nonce_bytes)?,
            EncryptionAlgorithm::ChaCha20Poly1305 => self.decrypt_chacha(ciphertext, nonce_bytes)?,
        };
    ...
    fn decrypt_aes(&self, ciphertext: &[u8], nonce_bytes: &[u8]) -> Result<Vec<u8>, EncryptionError> {
        let key = Key::<Aes256Gcm>::generate(&mut OsRng);
        let cipher = Aes256Gcm::new(&key);
    ...
    fn decrypt_chacha(&self, ciphertext: &[u8], nonce_bytes: &[u8]) -> Result<Vec<u8>, EncryptionError> {
        let key = ChaChaKey::generate(&mut OsRng);
        let cipher = ChaCha20Poly1305::new(&key);
