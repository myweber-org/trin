...
pub fn generate_key_file(password: &str, output_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let password_hash = argon2.hash_password(password.as_bytes(), &salt)?;
    let mut output_file = fs::File::create(output_path)?;
    output_file.write_all(password_hash.to_string().as_bytes())?;
...
mod tests {
    ...
    fn test_key_generation() {
        ...
        generate_key_file(password, key_file.path()).unwrap();
        
        let key_content = fs::read_to_string(key_file.path()).unwrap();
        assert!(key_content.contains("argon2id"));
        assert!(key_content.contains("v=19"));
    }
}