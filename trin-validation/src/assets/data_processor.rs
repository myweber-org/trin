...
mod tests {
    ...
    fn test_metadata_filtering() {
        ...
        record1.add_metadata("source", "sensor_a");

        let mut record2 = DataRecord::new(2, 1625097601);
        record2.add_metadata("source", "sensor_b");

        let records = vec![record1, record2];
        let filtered = filter_records_by_metadata(&records, "source", "sensor_a");
...
impl DataProcessor {
    ...
    fn update_category_stats(&mut self, record: &DataRecord) {
        let entry = self.category_stats.entry(record.category.clone()).or_insert(0.0);
        *entry += record.value;
    }
...
mod tests {
    ...
    fn test_category_stats() {
        ...
        assert_eq!(processor.get_category_average("A"), Some(15.0));
        assert_eq!(processor.get_category_average("B"), Some(30.0));
