...
impl DataProcessor {
    ...
    pub fn export_summary(&self) -> HashMap<String, f64> {
        let mut summary = HashMap::new();
        
        for (category, stats) in &self.category_stats {
            summary.insert(category.clone(), stats.average_value);
        }
        
        summary.insert("total_records".to_string(), self.records.len() as f64);
        summary.insert("overall_average".to_string(), self.get_average_value());
...
mod tests {
    ...
    fn test_data_processing() {
        ...
        writeln!(temp_file, "Alice,30,50000.0").unwrap();
        writeln!(temp_file, "Bob,25,45000.0").unwrap();
        writeln!(temp_file, "Charlie,35,60000.0").unwrap();

        let processor = DataProcessor::new(',', true);
        let result = processor.process_file(temp_file.path(), |columns| {
            columns.len() == 3 && columns[0] != "Bob"
        });
        ...
        assert_eq!(data[1][0], "Charlie");

        let avg_salary = processor.calculate_column_average(&data, 2);
        assert_eq!(avg_salary, Some(55000.0));
    }
}