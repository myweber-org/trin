    ...
    fn substitute_env_vars(value: &str) -> String {
        ...
        while let Some(ch) = chars.next() {
            if ch == '$' && chars.peek() == Some(&'{') {
                ...
                let env_value = env::var(&var_name).unwrap_or_else(|_| {
                    format!("${{{}}}", var_name)
                });
...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        writeln!(file, "PATH=/opt/${{APP_SECRET}}/data").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("SECRET"), Some(&"mysecret123".to_string()));
        assert_eq!(config.get("PATH"), Some(&"/opt/mysecret123/data".to_string()));
    }
}