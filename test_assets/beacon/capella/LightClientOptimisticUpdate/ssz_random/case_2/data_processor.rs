...
mod tests {
    ...
    fn test_data_processor() {
        ...
        writeln!(temp_file, "2,test2,20.0,2023-01-02").unwrap();
        writeln!(temp_file, "3,test3,5.5,2023-01-03").unwrap();
        ...
        assert!(avg.is_some());
        assert!((avg.unwrap() - 12.0).abs() < 0.001);
...
mod tests {
    ...
    fn test_merge_datasets() {
        ...
        let merged_data = processor.data.get("merged").unwrap();
        assert_eq!(merged_data, &vec![1.0, 2.0, 3.0, 4.0]);
    }
}