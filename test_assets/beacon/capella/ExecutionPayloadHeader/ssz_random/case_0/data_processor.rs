...
impl DataRecord {
    pub fn new(id: u32, value: f64, category: &str, timestamp: &str) -> Self {
        Self {
            id,
            value,
    ...
    pub fn is_valid(&self) -> bool {
        !self.category.is_empty() && self.value >= 0.0 && self.value <= 1000.0
    }
}
...
impl DataProcessor {
    ...
    pub fn load_from_csv<P: AsRef<Path>>(&mut self, path: P) -> Result<usize, Box<dyn Error>> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            let record = DataRecord::new(id, value, parts[2], parts[3]);
            if record.is_valid() {
...
mod tests {
    ...
    fn test_record_validation() {
        let valid_record = DataRecord::new(1, 100.5, "A", "2024-01-01");
        assert!(valid_record.is_valid());

        let invalid_value = DataRecord::new(2, -10.0, "B", "2024-01-01");
        assert!(!invalid_value.is_valid());

        let invalid_category = DataRecord::new(3, 50.0, "", "2024-01-01");
    ...
    fn test_data_processing() {
        ...
        writeln!(temp_file, "1,100.5,A,2024-01-01").unwrap();
        writeln!(temp_file, "2,200.0,B,2024-01-02").unwrap();
        writeln!(temp_file, "3,300.2,A,2024-01-03").unwrap();
        ...
        assert_eq!(stats.0, 100.5);
        assert_eq!(stats.1, 300.2);
    }
}