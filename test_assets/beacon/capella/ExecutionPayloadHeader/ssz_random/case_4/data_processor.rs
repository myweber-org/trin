...
impl DataProcessor {
    ...
    pub fn load_from_csv(&mut self, filepath: &str) -> Result<(), Box<dyn Error>> {
        ...
        self.metadata.insert("source_file".to_string(), filepath.to_string());
        self.metadata.insert("records_processed".to_string(), line_count.to_string());
    ...
    fn parse_header(&mut self, header_line: &str) {
        ...
        self.metadata.insert("column_count".to_string(), columns.len().to_string());
        
        if columns.len() > 0 {
            self.metadata.insert("first_column".to_string(), columns[0].to_string());
        }
    }
    ...
    pub fn extract_column(&self, records: &[Vec<String>], column_index: usize) -> Result<Vec<String>, String> {
        ...
        for record in records {
            if column_index >= record.len() {
                return Err(format!("Column index {} out of bounds for record", column_index));
            }
...
impl DataRecord {
    ...
    pub fn normalize(&self, max_value: f64) -> Result<f64, DataError> {
        if max_value <= 0.0 || !max_value.is_finite() {
            return Err(DataError::TransformationError(
                "Max value must be positive and finite".to_string()
            ));
        }
        
        if self.value > max_value {
            return Err(DataError::TransformationError(
                format!("Value {} exceeds maximum {}", self.value, max_value)
            ));
        }
...
pub fn process_records(records: &[DataRecord], multiplier: f64) -> Vec<Result<DataRecord, DataError>> {
    records
        .iter()
...
pub fn validate_records(records: &[DataRecord]) -> Result<(), DataError> {
    for record in records {
        if record.id == 0 {
            return Err(DataError::InvalidId);
        }
