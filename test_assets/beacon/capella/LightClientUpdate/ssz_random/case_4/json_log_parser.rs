...
mod tests {
    ...
    fn test_parser_with_filters() {
        ...
        writeln!(temp_file, r#"{{"level": "error", "message": "Disk full", "timestamp": "2023-10-01T12:00:00Z"}}"#).unwrap();
        writeln!(temp_file, r#"{{"level": "info", "message": "System started", "timestamp": "2023-10-01T11:00:00Z"}}"#).unwrap();
        writeln!(temp_file, r#"{{"level": "error", "message": "Network timeout", "timestamp": "2023-10-01T12:05:00Z"}}"#).unwrap();

        let results = parser.parse_file(temp_file.path()).unwrap();
        assert_eq!(results.len(), 2);
        assert_eq!(results[0].get("message").unwrap(), &json!("Disk full"));
        assert_eq!(results[1].get("message").unwrap(), &json!("Network timeout"));
    }
}