...
impl DataProcessor {
    ...
    pub fn load_from_csv<P: AsRef<Path>>(&mut self, path: P) -> Result<(), Box<dyn Error>> {
        ...
        for (line_num, line) in lines.enumerate() {
            ...
            if parts.len() != 3 {
                return Err(format!("Invalid number of columns at line {}", line_num + 2).into());
            }

            let id = parts[0].parse::<u32>()
                .map_err(|e| format!("Invalid ID at line {}: {}", line_num + 2, e))?;
            
            let value = parts[1].parse::<f64>()
                .map_err(|e| format!("Invalid value at line {}: {}", line_num + 2, e))?;
            
            let category = parts[2].to_string();

            match DataRecord::new(id, value, category) {
                Ok(record) => self.records.push(record),
                Err(e) => return Err(format!("Validation error at line {}: {}", line_num + 2, e).into()),
            }
        }
