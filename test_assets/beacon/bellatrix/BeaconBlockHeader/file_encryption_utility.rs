...
impl FileEncryptor {
    ...
    pub fn decrypt_file(&self, input_path: &Path, output_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        File::create(output_path)?.write_all(&plaintext)?;
...
pub fn generate_key_file(key_path: &Path, password: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    key_file.write_all(salt.as_str().as_bytes())?;
...
mod tests {
    ...
    fn test_key_generation() {
        ...
        generate_key_file(key_file.path(), password).unwrap();
        
        let key_data = fs::read_to_string(key_file.path()).unwrap();
        assert!(key_data.contains("argon2"));
        assert!(key_data.len() > 50);
    }
}