...
mod tests {
    ...
    #[test]
    fn test_env_substitution() {
        env::set_var("CUSTOM_ENV", "production");
        
        let mut file = NamedTempFile::new().unwrap();
        writeln!(file, "ENVIRONMENT=$CUSTOM_ENV").unwrap();
        writeln!(file, "STATIC_VALUE=test").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("ENVIRONMENT").unwrap(), "production");
        assert_eq!(config.get("STATIC_VALUE").unwrap(), "test");
    }
}