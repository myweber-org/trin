...
mod tests {
    ...
    fn test_encryption_roundtrip() {
        ...
        let plain_path = "test_plain.txt";
        let enc_path = "test_encrypted.bin";
        let dec_path = "test_decrypted.txt";

        fs::write(plain_path, test_data).unwrap();
        encrypt_file(plain_path, enc_path).unwrap();
        decrypt_file(enc_path, dec_path).unwrap();

        let decrypted = fs::read(dec_path).unwrap();
        assert_eq!(test_data.to_vec(), decrypted);

        fs::remove_file(plain_path).unwrap_or_default();
        fs::remove_file(enc_path).unwrap_or_default();
        fs::remove_file(dec_path).unwrap_or_default();
    }
}