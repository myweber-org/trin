...
impl FileEncryptor {
    pub fn new_from_password(password: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let cipher_key = aes_gcm::Key::<Aes256Gcm>::from_slice(&key.as_bytes()[..32]);
...
mod tests {
    ...
    fn test_file_encryption() -> io::Result<()> {
        ...
        encrypt_file(input_file.path(), output_file.path(), Some(0x42))?;
        decrypt_file(output_file.path(), decrypted_file.path(), Some(0x42))?;
