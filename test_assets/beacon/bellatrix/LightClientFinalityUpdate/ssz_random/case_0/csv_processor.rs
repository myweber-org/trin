...
impl CsvProcessor {
    ...
    pub fn process_file<P: AsRef<Path>>(&self, path: P) -> Result<Vec<Vec<String>>, Box<dyn Error>> {
        ...
        for line in reader.lines() {
            ...
            if self.config.selected_columns.is_empty() {
                ...
                let filtered: Vec<String> = self.config.selected_columns
                    .iter()
    ...
    pub fn process_and_print<P: AsRef<Path>>(&self, path: P) -> Result<(), Box<dyn Error>> {
        let data = self.process_file(path)?;
        
        for row in data {
            println!("{}", row.join(&self.config.delimiter.to_string()));
        }
