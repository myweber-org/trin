...
mod tests {
    ...
    fn test_data_processor() {
        ...
        writeln!(temp_file, "2,item2,5.0,false").unwrap();
        writeln!(temp_file, "3,item3,15.0,true").unwrap();

        let count = processor.load_from_csv(temp_file.path()).unwrap();
        assert_eq!(count, 3);
        assert_eq!(processor.count_records(), 3);

        let filtered = processor.filter_by_value(7.0);
        assert_eq!(filtered.len(), 2);

        let average = processor.calculate_average().unwrap();
        assert!((average - 10.166666666666666).abs() < 0.0001);
