...
mod tests {
    ...
    fn test_validation() {
        let mut config = Config::new();
        config.settings.insert("required_key".to_string(), "value".to_string());
        
        assert!(config.validate_required(&["required_key"]).is_ok());
        assert!(config.validate_required(&["timeout"]).is_ok());
        
        let result = config.validate_required(&["missing_key"]);
