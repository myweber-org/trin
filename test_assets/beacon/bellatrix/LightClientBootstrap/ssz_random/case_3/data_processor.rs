...
mod tests {
    ...
    fn test_data_processor() {
        ...
        processor.add_record(DataRecord::new(1, 100.0, "Category1"));
        processor.add_record(DataRecord::new(2, 200.0, "Category2"));
        processor.add_record(DataRecord::new(3, -50.0, "Category1"));
        
        assert_eq!(processor.get_average_value(), 150.0);
        assert_eq!(processor.get_valid_records().len(), 2);
        
        let counts = processor.count_by_category();
        assert_eq!(counts.get("Category1"), Some(&1));
        assert_eq!(counts.get("Category2"), Some(&1));
    }
    ...
    fn test_csv_loading() -> Result<(), Box<dyn Error>> {
        ...
        writeln!(temp_file, "3,-50.0,TypeA")?;
        
        let mut processor = DataProcessor::new();
        processor.load_from_csv(temp_file.path())?;
        
        assert_eq!(processor.get_average_value(), 150.4);
        assert_eq!(processor.get_valid_records().len(), 2);
