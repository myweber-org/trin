...
impl DataProcessor {
    ...
    pub fn process_data(&mut self, dataset_name: String, data: Vec<f64>) -> Result<Vec<f64>, String> {
        if data.is_empty() {
        ...
        for rule in &self.validation_rules {
            if rule.required && data.iter().any(|&x| x.is_nan()) {
                return Err(format!("Validation failed for {}: NaN values found", rule.field_name));
            }

            if let Some(&value) = data.iter().find(|&&x| x < rule.min_value || x > rule.max_value) {
                return Err(format!(
                    "Value {} out of range [{}, {}] for field {}",
                    value, rule.min_value, rule.max_value, rule.field_name
                ));
            }
        }
        ...
        self.cache.insert(dataset_name, processed_data.clone());
...
mod tests {
    ...
    fn test_data_processing() {
        ...
        let result = processor.process_data("test_data".to_string(), data);
    ...
    fn test_validation_failure() {
        ...
        let result = processor.process_data("invalid_data".to_string(), data);
...
mod tests {
    ...
    fn test_data_processing() {
        ...
        writeln!(temp_file, "category_a,18.9,30.2").unwrap();
        ...
        assert!(!top_categories.is_empty());
...
impl DataProcessor {
        ...
        F: Fn(f64) -> f64,
    {
        ...
        self.recalculate_category_totals();
    }

    fn recalculate_category_totals(&mut self) {
...
mod tests {
    ...
    fn test_value_transformation() {
        ...
        assert_eq!(processor.get_category_total("Test"), 200.0);
    }
}
...
impl DataRecord {
    ...
    pub fn normalize_values(&mut self) -> Result<(), ProcessingError> {
        ...
        if std_dev.abs() < f64::EPSILON {
            return Err(ProcessingError::TransformationFailed("Cannot normalize: standard deviation is zero".to_string()));
        }
...
pub fn filter_records_by_threshold(records: &[DataRecord], key: &str, threshold: f64) -> Vec<DataRecord> {
    records.iter()
        .filter(|record| {
...
mod tests {
    ...
    fn test_record_creation() {
        ...
        record.add_metadata("sensor", "sensor_001");

        assert_eq!(record.id, 1);
        assert_eq!(record.timestamp, 1234567890);
        assert_eq!(record.values.get("temperature"), Some(&25.5));
        assert_eq!(record.metadata.as_ref().unwrap().get("sensor"), Some(&"sensor_001".to_string()));
    }
    ...
    fn test_filtering() {
        ...
        records[0].add_value("score", 50.0);
        records[1].add_value("score", 75.0);
        records[2].add_value("score", 100.0);

        let filtered = filter_records_by_threshold(&records, "score", 75.0);
