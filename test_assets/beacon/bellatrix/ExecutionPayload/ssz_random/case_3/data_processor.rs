...
impl DataProcessor {
    ...
    pub fn load_from_csv(&mut self, file_path: &str) -> Result<usize, Box<dyn Error>> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            let valid = id > 0 && value > 0.0 && !category.is_empty();
...
mod tests {
    ...
    fn test_data_processing() {
        ...
        assert_eq!(stats.0, 4); // total records
        assert_eq!(stats.1, 3); // valid records
        
        let average = processor.calculate_average();
        assert!((average - 15.3).abs() < 0.001);
...
fn add_processing_metadata(record: DataRecord) -> Result<DataRecord, ProcessingError> {
    ...
    new_metadata.insert("processed_timestamp".to_string(), chrono::Utc::now().timestamp().to_string());
