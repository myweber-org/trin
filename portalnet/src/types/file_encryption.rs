...
pub fn decrypt_file(input_path: &Path, output_path: &Path, password: &str) -> Result<(), String> {
    ...
    let salt = encrypted_data[..SALT_LEN].try_into().unwrap();
    let iv = encrypted_data[SALT_LEN..SALT_LEN + IV_LEN].try_into().unwrap();
    let ciphertext = encrypted_data[SALT_LEN + IV_LEN..].to_vec();
