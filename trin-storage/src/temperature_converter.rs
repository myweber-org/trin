...
mod tests {
    ...
    fn test_fahrenheit_to_kelvin() {
        assert!((fahrenheit_to_kelvin(32.0) - 273.15).abs() < f64::EPSILON);
        assert!((fahrenheit_to_kelvin(-459.67) - 0.0).abs() < 0.01);
    }
    ...
    fn test_kelvin_to_fahrenheit() {
        assert!((kelvin_to_fahrenheit(273.15) - 32.0).abs() < f64::EPSILON);
