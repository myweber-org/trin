...
pub fn generate_key_file(password: &str, output_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let password_hash = argon2.hash_password(password.as_bytes(), &salt)?;
    let key_hash = password_hash.to_string();
    
    fs::write(output_path, key_hash)?;
    Ok(())
}

pub fn verify_password(password: &str, key_file_path: &Path) -> Result<bool, Box<dyn std::error::Error>> {
    let key_hash = fs::read_to_string(key_file_path)?;
